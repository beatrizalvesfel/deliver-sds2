...
export default function OrderDetails({ route }: Props) {
  ...
  const handleCancel = () => {
    navigatoin.navigate('Orders');
  }

  const handleConfirmDelivery = () => {
      confirmDelivery(order.id)
      .then(() => {
          Alert.alert(`Pedido ${order.id} confirmando com sucesso!`)
          navigatoin.navigate('Orders');
      })
      .catch(() => {
          Alert.alert(`Houve erro ao confrimar o pedido ${order.id}`)
      })
  }
    ...
    return (
        <>
            ...
            <View style={styles.container}>
                ...
                <RectButton style={styles.button}>
                    <Text style={styles.buttonText} onPress={handleCancel}>CANCELAR</Text>
                </RectButton>
            </View>
        </>
    );
